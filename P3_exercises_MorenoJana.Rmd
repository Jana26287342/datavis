...
library(reshape2)
exp.long <- melt(expressionData, id.vars= c("subject", "treatment"))
colnames(exp.long)[3:4] <- c("gene", "expression")
...
# Basic ggplot
library(ggplot2)
heatmap_1 <- ggplot(data=exp.long, aes(x=subject, y=gene, fill=log.expression)) + 
  geom_tile()
heatmap_1
```
...
```{r}
# Faceting
faceting_heatmap <- heatmap_2 + facet_grid(~treatment, scales = "free_x", space = "free_x")
faceting_heatmap
```
...
```{r}
# Change scale colour
colour_heatmap <- faceting_heatmap +
  scale_fill_viridis()
colour_heatmap
```
...
The over-expressed genes are: IFNA13, IFNA16, IFNA2, IFNA5 and IFNW1.
&emsp;
...
manhattanPlot3 <- manhattanPlot2 + labs(title="My Manhattan Plot", x= "Chromosome", y = "-log10(P)")
manhattanPlot3
```
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
The previous function is used to add text to the plot, in particular to annotate the name of the significant SNPs. We are using the P values, the significance and the SNPs.
&emsp;
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
The significant SNPs are:
``` r gwasData[gwasData$P<sig,] ```
...
```{r}
# Basic boxplot
box_1 <- ggplot(ToothGrowth, aes(x=supp, y=len)) +
  geom_boxplot() +
  labs(x="Delivery methods", y= "Tooth length")
box_1
```
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
As the p value is higher than 0.05, we fail to reject the null hypothesis. So, there is no statistically significant difference in tooth growth when comparing the two delivery methods at the 5% significance level.
&emsp;
...
```{r warning=F}
# Basic boxplot
box_3 <- ggplot(ToothGrowth, aes(x=as.factor(dose), y=len)) +
  geom_boxplot() +
  labs(x ="Dose of vitamin C", y = "Tooth growth")
box_3
```
...
# Basic boxplot with + geom_signif
box_4 <- box_3 + 
  geom_signif(comparisons = list(c("0.5", "1")), map_signif_level=FALSE, y_position = 29) + 
  geom_signif(comparisons = list(c("1", "2")), map_signif_level=FALSE, y_position = 31) + geom_signif(comparisons = list(c("0.5", "2")), map_signif_level=FALSE)
box_4
```
...
```{r warning=F}
# Customization
box_5<-ggplot(ToothGrowth, aes(x=as.factor(dose), y=len, fill=as.factor(dose)))+
  geom_boxplot() +
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
As all the p values are lower than 0.05, tooth growth increase significantly as the dose increases.
&emsp;
</div>
