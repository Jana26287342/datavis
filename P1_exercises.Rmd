...
Shape: Using shape instead of colour we can see that our plot changes, instead of coloring the points, now we have different shapes (triangles, circles, and squares). Also, we have a legend. 

Size: This time, when we change to variable size, we can see that instead of coloring the points, we have a change in the size. If we have a point 4 is smaller than point 8, that it is more bigger, we can see it in the plot. Also, we have a legend.
 
:::
...
::: {style="background-color:#F0F0F0"}
#####  <i class="fas fa-comment-dots"></i> Answer:
qsec using colour: We obtain different colours depending on the range values (e.g. if we have a value around 15.0, it will be painted with dark blue)

qsec using shape: This time, we obtain an error. This is because shape works with exact numbers, while qsec is supposed to work with ranges.
 
:::
...
::: {style="background-color:#F0F0F0"}
#####  <i class="fas fa-comment-dots"></i> Answer:
We have to use  geom_bar(position = "fill"). The idea of this command is to give proportions at every x-position. So, the bar plot will be standardized and the bars will have the same height.
 
:::
...
::: {style="background-color:#F0F0F0"}
#####  <i class="fas fa-comment-dots"></i> Answer:
If I want to compare the total amount of cars with each cylinder category I would use the graph with no specific position, in other words, I will put: geom_bar().
If I want to know the relative abundance of each type of transmission, I will use geom_bar(position= "fill"). This is because it is standardized, and we have a relative view of what we have.
 
:::
``
...
::: {style="background-color:#F0F0F0"}
#####  <i class="fas fa-comment-dots"></i> Answer:
We can see that in the facet_wrap() we obtain the plots in vertical and contiguous. While, using the facet_grid() we obtain the plots horizontally.
 
:::
...
::: {style="background-color:#F0F0F0"}
#####  <i class="fas fa-comment-dots"></i> Answer:
The best subplot configuration to compare the distribution is the one generated using the facet_grid(), this is because it is more visual.
 
:::
...
p <- ggplot(data = iris, mapping = aes(x = Sepal.Width, fill = Species)) +
  ...
  facet_grid(Species ~ .)
#scale_fill_brewer
p + scale_fill_brewer( name = "Species name") +
  scale_x_continuous(name = "Sepal width") + 
  ggtitle("Iris sepal variation")

#scale_fill_viridis_d
p + scale_fill_viridis_d( name = "Species name") +
  scale_x_continuous(name = "Sepal width") + 
  ggtitle("Iris sepal variation")
...
::: {style="background-color:#F0F0F0"}
#####  <i class="fas fa-comment-dots"></i> Answer:
I will use palette "scale_fill_viridis_d to ensure that colourblind people can distinguish the colors. This is because we can observe that the colour are different, and we cannot confuse them. They are specially designed for this people.
 
:::
...
```{r}
p+labs(
    title = "Iris sepal variation",
    subtitle = "Setosa, Versicolor and Virginica species",
...
::: {style="background-color:#F0F0F0"}
#####  <i class="fas fa-comment-dots"></i> Answer:
Subtitle --> The text of the subtitle that is below the title.
Caption --> The text of the caption. It is displayed in the bottom-right of the plot (default).
Tag --> The text of the tag label, it is displayed at the top-left of the plot (default).

They are used to give additional information to the plots.
 
:::
...
::: {style="background-color:#F0F0F0"}
#####  <i class="fas fa-comment-dots"></i> Answer:
The theme that maximizes the data-ink ratio is theme_minimal(). This is because as it has no background annotations, we can understand better the data.
 
:::
...
```{r answer 5.1}
ggsave(filename = "plot_5.png", plot = p, width = 6, height = 4)
#install.packages("svglite")
#install.packages("systemfonts")
library(svglite)
ggsave(filename = "plot_5.1.svg", plot= p,  width = 6, height = 4)
```

::: {style="background-color:#F0F0F0"}
#####  <i class="fas fa-comment-dots"></i> Answer:
The big difference is the memory size, for a raster format, the size is much larger than for a vector format.
 
:::
...
p1
#we will use png
ggsave(filename = "plot_6.png", plot = p1, width = 6, height = 4)
```
